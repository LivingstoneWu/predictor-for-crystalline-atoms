...
# The first part of the model is a sigmoid classifier that apply weight of 118 to true classes to solve inherent
# class imbalance that most of the elements are not present in the dataset. By changing the class weight, the 
# robustness of the model is improved.

# path for the k-NN distance data
dataPath='/Users/j.s.k/Downloads/extractor/AlSiPS_distance_data.csv'
# A dictionary for the periodic dictionary
...
# A method to test if the element is in the list
def element_in_list(element, list):
    for i in list:
        if (i==element): return True
    return False

# method to turn the character representation of element into labels represented by integers
...
model=keras.Sequential([
    layers.Dense(64, activation='sigmoid',name='layer_1',input_shape=(5,)),
...
history=model.fit(X_train,y_train,batch_size=64,epochs=60,class_weight=class_weights)
model.save('./first_part_model')
